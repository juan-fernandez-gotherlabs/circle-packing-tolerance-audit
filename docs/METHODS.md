...
`scripts/refine_exact.py` stabilizes the contact root in binary64, performs
Newton iterations at 120 decimal digits, rounds to 90 places, and applies the
conservative uniform radius reduction. The resulting CSV is then checked
independently by the rational verifier.
...
The frozen audit recomputed the score and all 429 constraints for every
complete public witness it could obtain. Serialized decimals were interpreted
exactly and binary64 arrays as their exact IEEE-754 values. Claims without a
downloadable witness were documented but not ranked. `data/provenance.json`
records the upstream source, identifier, detected license status, and
redistribution decision for every candidate.

## Provenance and evidence archive

The three primary certificates and the historical files were recovered from
the authenticated ChatGPT conversation “Mejorar resultado matemático”
...
They are preserved in the release attachment
`circle-packing-full-evidence-v1.1.0.zip`, which contains its own per-file
manifest. `python scripts/build.py --evidence-archive <path>` reconstructs that
archive from the immutable `v1.0.0` tag.
...
3. “Rank 1” means first only within the frozen public corpus dated 2026-08-06
   and only under the same tolerance contract.
