...
python3 -c 'import sys; expected=(3, 12); actual=sys.version_info[:2]; raise SystemExit(0 if actual == expected else f"ERROR: release verification requires CPython 3.12, found {sys.version.split()[0]}")'
